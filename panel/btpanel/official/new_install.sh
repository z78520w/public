...
check_port(){
	...
	do
		clear
		echo && read -p "请输入将安装宝塔面板的端口(默认:8888)：" panelPort
		[ -z "${panelPort}" ] && panelPort=8888
...
check_pip(){
	clear
	if [[ ! `pip -V|awk -F '(' '{print $2}'` =~ 'python 2.7' ]]; then
		...
		if [[ -n $python_path ]]; then
			ln -s $python_path /usr/local/bin/pip
			ln -s $python_path /usr/bin/pip
